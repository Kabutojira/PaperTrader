...
5. Update the security page, one-line security summary, source records, and comparable assessment
   through the validated CLI when the check changes or reconfirms time-sensitive state. Every
   succeeded quick check must invoke `scripts/papertrader research assessment upsert --request
   <unique-request-path>` and publish exactly one immutable assessment version whose `run_id` is
   the current run and whose source operation is this quick check, even when the conclusion is
   unchanged. Merely writing the request file does not apply it. Run `research security-context`
   again after the upsert and confirm both identities before writing the result manifest.
...
actual files changed, exact CLI receipts, the accepted same-run assessment version, and the one
created full-research operation ID when escalation occurred. The manifest describes completed
work, not proposed edits. An assessment request that was written but not invoked is proposed work
and requires a failed result.
